...
![文字編集に入ったところ。選択枠の見た目が変わり、右の「文字書式」が文字ごとの書式に切り替わる](docs/images/text-edit.png)
...
![キーボードショートカットの一覧。Mac と Windows / Linux をタブで切り替えられる](docs/images/shortcuts.png)

ツールバー右の「**ヘルプ**」(`⌘/`、Windows は `Ctrl+/` か `F1`)で一覧が開きます。
表記は実行環境に合わせて出ますが、タブで **Mac / Windows を切り替えて**確かめられます。
キーは PowerPoint に合わせてあり、macOS でも `Ctrl` を `⌘` の代わりに使えます。

### テーマを変える

![ダークテーマ。アプリの枠だけが暗くなり、スライドの見た目は変わらない](docs/images/dark.png)

ステータスバー右端の「テーマ」で **ライト(既定) / ダーク / システム(OS に追従)** を選べます。

**変わるのはアプリ自身の見た目だけ**です。スライド・選択枠・ハンドル・再生画面・
書き出す HTML は、どれも変わりません。選んだ配色はペイン幅と同じくこの端末に残ります
(デッキには入りません)。
